                    ...
                    <label className="flex items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-white px-3 py-3 text-sm dark:border-slate-700 dark:bg-slate-950">
                        <div className="min-w-0">
                            <div className="flex items-center gap-2 font-semibold text-slate-800 dark:text-slate-100">
                                ...
                                <InfoTip text="Du bekommst E-Mails zu Einladungen (z.B. Einladung annehmen)." />
                            </div>
                            <div className="text-xs text-slate-500 dark:text-slate-400">E-Mail bei Einladungen.</div>
                        </div>
                        <input
                            type="checkbox"
                            checked={Boolean(settings.inviteEmailsEnabled)}
                            onChange={(event) => setSettings((current) => ({ ...current, inviteEmailsEnabled: event.target.checked }))}
                            className="h-4 w-4 rounded border-slate-300 text-sky-600 focus:ring-sky-500"
                        />
                    </label>
                </div>
...
const AdminToolsPanel = ({ currentUser }) => {
    ...
    const [monitorStatus, setMonitorStatus] = React.useState(null);
    ...
    const loadAdminData = React.useCallback(async () => {
        ...
        try {
            const [statsRes, logsRes, digestRes, monitorRes] = await Promise.all([
                authFetch('/api/admin/stats'),
                authFetch('/api/admin/logs?limit=200'),
                authFetch('/api/admin/digest/status'),
                authFetch('/api/admin/monitor/status'),
            ]);
            ...
            if (!monitorRes.ok) throw new Error(monitorData.error || 'Monitoring-Status konnte nicht geladen werden');

            setStats(statsData);
            setLogs(Array.isArray(logsData.entries) ? logsData.entries.slice().reverse() : []);
            setDigestStatus(digestData?.status ?? null);
            setMonitorStatus(monitorData?.status ?? null);
        } catch (error) {
            console.error(error);
    ...
    return (
        <div className="space-y-4">
            ...
            <SidebarSection title="Betriebscheck" subtitle="Healthcheck, Fehlerquote, Speicher, Digest und Datenbereinigung.">
                {!monitorStatus ? (
                    <div className="rounded-xl border border-dashed border-slate-300 px-3 py-3 text-xs text-slate-500 dark:border-slate-700 dark:text-slate-400">
                        Noch kein automatischer Lauf. Der Check startet täglich gegen 07:00 Uhr.
                    </div>
                ) : (
                    <div className={`rounded-xl border px-3 py-3 text-xs ${monitorStatus.activeIssues?.length ? 'border-rose-200 bg-rose-50 text-rose-900 dark:border-rose-900/40 dark:bg-rose-950/30 dark:text-rose-100' : 'border-emerald-200 bg-emerald-50 text-emerald-900 dark:border-emerald-900/40 dark:bg-emerald-950/30 dark:text-emerald-100'}`}>
                        <div className="flex items-center justify-between gap-3">
                        ...
                        {monitorStatus.activeIssues?.map((issue) => (
                            <div key={issue.key} className="mt-2 rounded-lg border border-current/20 bg-white/40 px-2 py-2 text-[11px] leading-snug dark:bg-black/10">
                                <strong>{issue.title}</strong><br />{issue.detail}
                            </div>
                        ))}
                    </div>
                )}
...
const InvitationPanel = ({ currentCalendar }) => {
    ...
    const loadInvitations = React.useCallback(async () => {
        ...
        try {
            ...
            const data = await response.json();
            if (!response.ok) throw new Error(data.error || 'Einladungen konnten nicht geladen werden');
            setInvitations(Array.isArray(data.invitations) ? data.invitations : []);
        } catch (error) {
            console.error(error);
            toast.error(error.message || 'Einladungen konnten nicht geladen werden');
            setInvitations([]);
        } finally {
    ...
    React.useEffect(() => {
        ...
        loadMembers();
        loadInvitations();
    }, [loadInvitations, loadMembers, currentCalendar?.id, isOwner]);

    React.useEffect(() => {
        if (!isOwner) return;
        loadInvitations();
    }, [isOwner, loadInvitations, currentCalendar?.id]);
    ...
    const roleLabel = (value) => {
        if (value === 'editor') return 'Bearbeiter';
    ...
    const statusLabel = (value) => {
        ...
        if (value === 'revoked') return 'beendet';
        return 'pending';
    };
    ...
    const createInvitation = async () => {
        ...
        try {
            const response = await authFetch('/api/invitations', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ role, expiresInDays, expiresMode }),
            });
            const data = await response.json();
    ...
    const sendInviteEmail = async () => {
        ...
        try {
            const response = await authFetch('/api/invitations/send-email', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ role, expiresInDays, expiresMode, email: inviteEmail.trim() }),
            });
            const data = await response.json();
            ...
            if (data?.emailSkipped) {
                toast.success('Einladung erstellt. E-Mail-Versand ist beim Empfänger deaktiviert – bitte Link kopieren.');
            } else {
    ...
    const revokeInvitation = async (inviteId) => {
        const ok = window.confirm('Möchtest du diese Einladung wirklich beenden?');
        if (!ok) return;
        try {
            const response = await authFetch(`/api/invitations/${inviteId}`, { method: 'DELETE' });
            const data = await response.json();
            if (!response.ok) throw new Error(data.error || 'Beenden fehlgeschlagen');
            toast.success('Einladung beendet');
            await loadInvitations();
        } catch (error) {
            console.error(error);
            toast.error(error.message || 'Beenden fehlgeschlagen');
        }
    };
            ...
            <div className="space-y-3">
                <div className="grid gap-2">
                    ...
                    <label className="grid gap-1 text-xs font-semibold text-slate-600 dark:text-slate-300">
                        ...
                        <select
                            value={expiresMode}
                            onChange={(event) => setExpiresMode(event.target.value)}
                            className="h-10 rounded-xl border border-slate-200 bg-white px-3 text-sm text-slate-800 shadow-sm dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100"
                        >
                            <option value="days">X Tage</option>
                            ...
                            members.map((m) => (
                                <div key={m.id} className="flex items-center justify-between gap-2 rounded-lg border border-slate-200 bg-slate-50 px-2 py-2 dark:border-slate-700 dark:bg-slate-900/60">
                                    <div className="min-w-0">
                                        ...
                                        <div className="truncate text-[11px] text-slate-500 dark:text-slate-400">{m.email}</div>
                                        <div className="text-[11px] text-slate-500 dark:text-slate-400">{roleLabel(m.role)}</div>
                                    </div>
                                    {m.role !== 'owner' && (
                                        <button
                                            type="button"
                ...
                <div className="rounded-xl border border-slate-200 bg-white p-3 text-xs text-slate-700 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-200">
                    <div className="flex items-center justify-between gap-2">
                        <div className="font-semibold">Einladungen</div>
                        <button
                            type="button"
                            ...
                            className="rounded-lg border border-slate-200 px-2 py-1 text-[11px] font-semibold text-slate-500 transition-colors hover:bg-slate-100 hover:text-slate-900 disabled:cursor-not-allowed disabled:opacity-60 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800 dark:hover:text-white"
                        >
                            {invitesLoading ? 'Lade…' : 'Aktualisieren'}
                        </button>
                    </div>

                    <div className="mt-2 space-y-2">
                        {invitations.length === 0 ? (
                            <div className="text-slate-500 dark:text-slate-400">Noch keine Einladungen erstellt.</div>
                        ) : (
                            invitations.map((inv) => (
                                <div key={inv.id} className="flex items-start justify-between gap-3 rounded-lg border border-slate-200 bg-slate-50 px-2 py-2 dark:border-slate-700 dark:bg-slate-900/60">
                                    <div className="min-w-0">
                                        <div className="flex flex-wrap gap-x-2 gap-y-1 text-[11px] text-slate-500 dark:text-slate-400">
                                            <span className="font-semibold text-slate-700 dark:text-slate-200">{statusLabel(inv.status)}</span>
                                            <span>·</span>
                                            <span>{roleLabel(inv.role)}</span>
                                            <span>·</span>
                                            <span>erstellt {formatDate(inv.createdAt)}</span>
                                            <span>·</span>
                                            <span>gültig bis {inv.expiresAt ? formatDate(inv.expiresAt) : 'unbegrenzt'}</span>
                                        </div>
...
const ProfilePanel = ({ currentUser, onLogout }) => {
    ...
    return (
        <div className="space-y-4">
            ...
            <SidebarSection title="Rechtliches" subtitle="Impressum, Datenschutz & Cookies.">
                <div className="grid gap-2">
                    <Link
                        to="/impressum"
                        className="w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-700 transition-colors hover:bg-slate-100 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-100 dark:hover:bg-slate-800"
            ...
            <SidebarSection title="Eltern" subtitle="Farben und regelmäßige freie Tage für Papa und Mama.">
                <div className="space-y-3">
                    ...
                    <details className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm text-slate-800 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100">
                        ...
                        <div className="mt-2 space-y-2">
                            {careData.ranges.length > 0 ? careData.ranges.map(renderRangeRow) : (
                                <div className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-xs text-slate-600 dark:border-slate-700 dark:bg-slate-900/70 dark:text-slate-300">
                                    Keine Betreuungstage eingetragen.
                                </div>
                            )}
                        </div>
                    </details>
